...
> 「三要素」即**上表的对应列**,不必另立一张表:
> **触发条件 = `触发` 列、Skill 加载 = `Skill` 列、质量门禁 = `门禁` 列。**
> (原先这里有一张 10×3 全 `<待填>` 的表,与上表内容重复且永远空着 —— 已删除。
> 需要给某阶段补充更细的触发/加载/门禁说明时,直接写在该阶段的小节里。)
>
> 机器可读的版本在 `harness/state/stages.json`(阶段 → 产出物),
> 由 `scripts/stage_gate.py` 与 `scripts/check-gates.sh` 消费。
