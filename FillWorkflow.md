...
>
> ⚠ **本仓已于 2026-09-11 封存（停止演进）**，本文档描述的填充流程
> **尚未在本仓被真正执行过一次**。它是为**存量项目**设计的：14 个 Agent 的输入
> 全是**既有产物**（事故 / Review / 架构 / DDL / 遗留代码）。
> **没有这些来源就不要开始填充** —— 缺来源时产出的是编造内容，
> 而"来源为空则拒绝"正是本仓第一道防编造闸。
> 现状与复活条件见 [`docs/project-status.md`](./docs/project-status.md)。
